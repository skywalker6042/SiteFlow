...
function OrgRow({ org, caps }: { org: Org; caps: OrgCaps }) {
  ...
  const [plan, setPlan]               = useState<PlanTier>(org.plan ?? 'trial')
    ...
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Link href={`/admin/orgs/${org.id}`} className="text-sm font-semibold text-gray-900 hover:text-teal-600 transition-colors">
              {org.name}
            </Link>
      ...
      {/* Plan */}
      <td className="px-4 py-3.5">
        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold capitalize ${PLAN_STYLES[plan]}`}>
          {plan}
        </span>
      </td>
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-1.5 justify-end">
          ...
          <button
            onClick={enterOrg}
            disabled={entering}
            className="text-xs font-medium text-teal-600 border border-teal-200 hover:border-teal-400 bg-teal-50 hover:bg-teal-100 px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-50"
          >
            {entering ? <Loader size={11} className="animate-spin" /> : <ExternalLink size={11} />} Enter
          </button>

          {/* Status dropdown — only shown if user has at least one management cap */}
          {(caps.canManageStatus || caps.canManagePlan || caps.canManageBilling) && (
            <>
              <button
                ref={btnRef}
                onClick={openMenu}
              ...
              {open && (
                ...
                >
                  {caps.canManageStatus && (
                    <>
                      <p className="px-3 py-1 text-[10px] text-gray-400 uppercase tracking-wide font-semibold">Status</p>
                  ...
                  {caps.canManagePlan && (
                    <>
                      ...
                      <p className="px-3 py-1 text-[10px] text-gray-400 uppercase tracking-wide font-semibold">Plan</p>
                      <DropItem label="Trial Plan" onClick={() => patch({ plan: 'trial' })} active={plan === 'trial'} />
                      <DropItem label="Core Plan"  onClick={() => patch({ plan: 'core' })}  active={plan === 'core'} />
                      <DropItem label="Pro Plan"   onClick={() => patch({ plan: 'pro' })}   active={plan === 'pro'} />
                      <div className="border-t border-gray-100 my-1" />
                      <div className="px-3 py-2">
                        ...
                        <div className="flex gap-1.5">
                          <input
                            type="number" min={1} placeholder="days" value={trialDays}
                            onChange={e => setTrialDays(e.target.value)}
                            onKeyDown={e => e.key === 'Enter' && setTrialExpiry()}
                            className="w-16 text-xs border border-gray-200 rounded-md px-2 py-1 focus:outline-none focus:border-teal-400"
                          />
                          <button onClick={setTrialExpiry} className="text-xs bg-teal-500 text-white px-2 py-1 rounded-md hover:bg-teal-600 transition-colors">Set</button>
                        </div>
                      </div>
                    </>
                  )}
                  {caps.canManageBilling && (
                    <>
                      <div className="border-t border-gray-100 my-1" />
                      <p className="px-3 py-1 text-[10px] text-gray-400 uppercase tracking-wide font-semibold">Billing</p>
...
export function OrgTable({ orgs, canManageStatus, canManagePlan, canManageBilling }: {
  orgs: Org[]
  canManageStatus: boolean
  canManagePlan: boolean
  canManageBilling: boolean
}) {
  const caps: OrgCaps = { canManageStatus, canManagePlan, canManageBilling }
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {orgs.map((org) => <OrgRow key={org.id} org={org} caps={caps} />)}
          </tbody>
        </table>
      </div>
    </div>
